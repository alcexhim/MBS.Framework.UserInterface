...
namespace MBS.Framework.UserInterface.Engines.GTK3.Controls
{
	...
	public class TabContainerImplementation : GTKNativeImplementation, ITabContainerControlImplementation
	{
		...
		public void NotebookAppendPage(TabContainer ctl, IntPtr handle, TabPage page, int indexAfter = -1)
		{
			...
			(Engine as GTK3Engine).RegisterControlHandle(page, new GTKNativeControl(hTabPage));
		...
		{
			TabContainer tbsParent = (((UIApplication)Application.Instance).Engine as GTK3Engine).GetControlByHandle(notebook) as TabContainer;
		...
		private static void switch_page(IntPtr /*GtkNotebook*/ notebook, IntPtr /*GtkWidget*/ page, uint page_num)
		{
			...
			TabContainer tc = (((UIApplication)Application.Instance).Engine as GTK3Engine).GetControlByHandle(notebook) as TabContainer;
