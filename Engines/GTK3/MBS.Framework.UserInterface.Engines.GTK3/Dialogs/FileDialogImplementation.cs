...
namespace MBS.Framework.UserInterface.Engines.GTK3.Dialogs
{
	...
	public class FileDialogImplementation : GTKDialogImplementation //, IFileDialogImplementation
	{
		...
		protected override GTKNativeControl CreateDialogInternal(Dialog dialog, List<Button> buttons)
		{
			...
			IntPtr handle = Internal.GTK.Methods.GtkFileChooserDialog.gtk_file_chooser_dialog_new(title, (Engine as GTK3Engine).CommonDialog_GetParentHandle(dlg), fca);
			...
			switch (dlg.Mode)
			{
				...
				case FileDialogMode.Save:
				{
					accept_button = (Engine as GTK3Engine).StockTypeToString(StockType.Save);
					break;
				}
				case FileDialogMode.SelectFolder:
				case FileDialogMode.Open:
				{
					accept_button = (Engine as GTK3Engine).StockTypeToString(StockType.Open);
					break;
				}
			}
