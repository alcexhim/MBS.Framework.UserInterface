	...
	public abstract class GTKDialogImplementation : GTKNativeImplementation
	{
		...
		private IntPtr Dialog_AddButton(Dialog dialog, IntPtr handle, Button button)
		{
			...
			*/
			...
			if (!(Engine as GTK3Engine).IsControlCreated(button))
				(Engine as GTK3Engine).CreateControl(button);

			buttonHandle = ((Engine as GTK3Engine).GetHandleForControl(button) as GTKNativeControl).Handle;
		...
		{
			Window window = ((((UIApplication)Application.Instance).Engine as GTK3Engine).GetControlByHandle(widget) as Window);
			if (window != null)
			{
			...
			(Engine as GTK3Engine).UnregisterControlHandle(ctl);
			return false;
		}
		...
		protected override NativeControl CreateControlInternal(Control control)
		{
			...
			Internal.GLib.Structures.Value val = new MBS.Framework.UserInterface.Engines.GTK3.Internal.GLib.Structures.Value(1);
			// }
			...
			{
				IntPtr hButtonDefault = ((Engine as GTK3Engine).GetHandleForControl(dialog.DefaultButton) as GTKNativeControl).Handle;
